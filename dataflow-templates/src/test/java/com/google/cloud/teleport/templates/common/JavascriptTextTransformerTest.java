...
public class JavascriptTextTransformerTest {
  ...
  private static final String TRANSFORM_FILE_PATH_WITH_CONFIGS =
          Resources.getResource(RESOURCES_DIR + "transformWithConfigs.js").getPath();
  ...
  /**
   * Test {@link JavascriptRuntime#invoke(String, String)} returns transformed data when a good javascript
  ...
  public void testInvokeGood() throws Exception {
    JavascriptRuntime javascriptRuntime =
        JavascriptRuntime.newBuilder()
            ...
            .build();
    String[] data = javascriptRuntime.invoke("{\"answerToLife\": 42}", "");
    Assert.assertArrayEquals(new String[]{"{\"answerToLife\":42,\"someProp\":\"someValue\"}"}, data);
  }

  /**
   * Test {@link JavascriptRuntime#invoke(String, String)} errors when undefined data returned from
   * javascript function.
   */
  @Test
  ...
  /**
   * Test {@link JavascriptRuntime#invoke(String, String)} returns
   *    transformed data when a good javascript with configurations
   * javascript function.
   */
  @Test
  public void testInvokeGoodWithConfigs() throws Exception {
    JavascriptRuntime javascriptRuntime =
            JavascriptRuntime.newBuilder()
                    .setFileSystemPath(TRANSFORM_FILE_PATH_WITH_CONFIGS)
                    .setFunctionName("transform")
                    .build();
    String[] data = javascriptRuntime.invoke("{\"answerToLife\": 42}", "{\"someConf\":\"someValue\"}");
    Assert.assertArrayEquals(new String[]{"{\"answerToLife\":42,\"someConf\":\"someValue\"}"}, data);
  }
